...
| `reveal-probe.ps1` | Reveal states per route (Phase 2+): how many `Reveal` elements are hidden 2.5 s after boot and where they sit relative to the fold, how many are still hidden after one scroll pass (expect 0), and the 404s seen during the visit. Pinned stages are excluded because their captions drive their own inline opacity from scroll refs. |
